...
(async () => {
    ...
    var revealDur = window.__TRANSITION_DUR__ || '.15s';
    document.documentElement.style.cssText = 'visibility:visible;opacity:1;transition:opacity ' + revealDur;
...
function showPage(p) {
    ...
    if (p === 'home') {
        ...
        currentRootScope = '';
        currentFiles = [];
        originalFolderFiles = []; // Clear original folder files
...
async function smartNavigate(path, title, orgUrl = SB_URL_ORG1, orgKey = SB_KEY_ORG1) {
    ...
    try {
        ...
        if (isTeacherFiles) {
            ...
            const items = await listPath(path, SB_URL_ORG3, SB_KEY_ORG3).catch(() => []);
            files = items.filter(i => i.id && i.name !== '.emptyFolderPlaceholder' && i.name.match(/\.(pdf|doc|docx)$/i))
                .map(f => ({ name: f.name, fullPath: `${path}/${f.name}`, size: f.metadata?.size, orgUrl: SB_URL_ORG3, orgKey: SB_KEY_ORG3 }));
        } else {
            ...
            const map1 = org1Items.filter(i => i.id && i.name !== '.emptyFolderPlaceholder' && i.name.match(/\.(pdf|doc|docx)$/i))
                .map(f => ({ name: f.name, fullPath: `${path}/${f.name}`, size: f.metadata?.size, orgUrl: SB_URL_ORG1, orgKey: SB_KEY_ORG1 }));
            const map2 = org2Items.filter(i => i.id && i.name !== '.emptyFolderPlaceholder' && i.name.match(/\.(pdf|doc|docx)$/i))
                .map(f => ({ name: f.name, fullPath: `${path}/${f.name}`, size: f.metadata?.size, orgUrl: SB_URL_ORG2, orgKey: SB_KEY_ORG2 }));
            const map3 = org3Items.filter(i => i.id && i.name !== '.emptyFolderPlaceholder' && i.name.match(/\.(pdf|doc|docx)$/i))
                .map(f => ({ name: f.name, fullPath: `${path}/${f.name}`, size: f.metadata?.size, orgUrl: SB_URL_ORG3, orgKey: SB_KEY_ORG3 }));

            // Merge and deduplicate by filename (keeping newest/last org if duplicate)
            const fileMap = new Map();
            [...map1, ...map2, ...map3].forEach(f => fileMap.set(f.name, f));
        ...
        renderFileObjects(currentFiles, browserContainer, false);
    ...
    if (!q) {
        ...
        if (currentPage === 'browser') {
            if (originalFolderFiles.length) {
                // Restore the original folder file list
                const browserTitleEl = document.getElementById('browser-title');
...
function runLocalSearch(q) {
    const container = document.getElementById('browser-container');
    // Filter from originalFolderFiles (not currentFiles) so search doesn't corrupt the folder list
    ...
    // Save search results but do NOT overwrite currentFiles (keep it as folder files)
    lastSearchResults = matches;
    lastSearchQuery = q;
    document.getElementById('browser-title').innerText =
        matches.length ? `"${q}" — ${matches.length} result${matches.length > 1 ? 's' : ''}` : `No results for "${q}"`;
...
window.onload = () => {
    ...
    const delay = window.__INTRO_TIMEOUT__ || 800;
    if (intro) setTimeout(() => intro.remove(), delay);
    showPage('home');
    loadGroqKeys();
...
function appendNavResults(matches, queryLabel) {
    ...
    avatar.innerHTML = `<img src="/ibilib/image/BILIBot.png" alt="BILIBot" style="width:100%;height:100%;object-fit:cover;border-radius:6px;">`;
    ...
    if (role === 'bot') {
        div.innerHTML = `
            <div class="bilibot-msg-avatar">
                <img src="/ibilib/image/BILIBot.png" alt="BILIBot" style="width:100%;height:100%;object-fit:cover;border-radius:6px;">
            </div>
...
async function sendBiliBot() {
    ...
    if (availableMatch) {
        ...
        try {
            ...
            av2.innerHTML = '<img src="/ibilib/image/BILIBot.png" alt="BILIBot" style="width:100%;height:100%;object-fit:cover;border-radius:6px;">';
