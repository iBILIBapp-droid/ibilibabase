...
(async () => {
  ...
  style.textContent = `
    ...
    /* ── MOBILE (<=768px): Inline inside mobile menu ── */
    @media (max-width: 768px) {
      ...
      .dropdown-header {
        display: none !important; /* hide header since user trigger already shows info */
      }
      #_user-dropdown .logout {
        width: 100%;
        justify-content: flex-start;
        padding: 12px;
        background: rgba(248, 113, 113, 0.1);
  ...
  bar.id = '_user-dropdown';
  bar.className = 'user-dropdown';
  bar.innerHTML = `
    ...
    <div class="dropdown-menu" id="_user-menu">
      ...
      <div id="_user-menu-items">
        <!-- Other modules (like Account Manager) can inject items here -->
      </div>
      <button class="dropdown-item logout" onclick="_doLogout()">
  ...
  function _mountAuthBar() {
    ...
    if (window.innerWidth > 768) {
      if (navActions) { navActions.appendChild(bar); return; }
      if (navInner) { navInner.appendChild(bar); return; }
    } else {
      if (mobileMenu) { mobileMenu.appendChild(bar); return; }
    }
  ...
  bar.addEventListener('click', (e) => {
    if (e.target.closest('#_user-trigger')) {
      e.stopPropagation();
